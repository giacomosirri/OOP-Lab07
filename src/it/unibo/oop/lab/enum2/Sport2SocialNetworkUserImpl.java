...
public class Sport2SocialNetworkUserImpl<U extends User> extends SocialNetworkUserImpl<U> {
	...
	public Set<Sport> getIndividualSports() { 
		...
		for (final Sport thisSport : this.getSports()) {
			if (thisSport.isIndividualSport()) {
				individualSports.add(thisSport);
			}
		}
	...
	public Set<Sport> getSportPracticedInPlace(Place p) {
		Set<Sport> thisPlaceSports = new HashSet<>();
		for (final Sport thisSport : this.getSports()) {
			if (thisSport.getPlace().equals(p)) {
				thisPlaceSports.add(thisSport);
			}
		}
		return thisPlaceSports;
	}
}
